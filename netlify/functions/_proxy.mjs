...
function rewriteRoots(body, prefix) {
  ...
  body = body.replace(/\b(href|src|action|poster)\s*=\s*'\/(?!\/)([^']*)'/gi, (_, a, r) => `${a}='${p(r)}'`);
  body = body.replace(/url\(\s*(["']?)\/(?!\/)([^"')\s]+)\1\s*\)/gi, (_, q, r) => `url(${q}${p(r)}${q})`);
  body = body.replace(/(["'`])\/(?!\/)([^"'`\\\s<>)]*)\1/g, (_, q, r) => `${q}${p(r)}${q}`);
  return body;
}
...
export async function proxy(request, options = {}) {
  ...
  try {
    ...
    const proxyBase = prefix ? publicOrigin + prefix : publicOrigin;
    const targetHost = new URL(target).host;
    const escapedHost = escapeRegExp(targetHost);

    const out = new Headers();
    res.headers.forEach((v, k) => {
      if (!SKIP_RESPONSE.has(k.toLowerCase())) out.set(k, v);
    });
